...
总体原因分析：accuracy高但是使用效果不好，负面情绪样本少，happy，surprised，calm多，这三个的高准确率拉高了总体的accuracy，使ai讨好这些label，失真

STEP5：  
1.更换了fer+数据集（增加了轻蔑情绪），并进行了数据清洗（包括但不限于，删除非三次元人类面部，标注明显错误的数据）  
2.调整了不同label数据的权重，增加样本量更少的负面情绪的权重  
学习效果：  
![img](images/step5.jpg)    
结果测评：（混淆矩阵）  
![img](images/step5_test.jpg)  
原因分析：  
1.训练集进行了数据增强，所以训练的准确率比验证集中要低，这是一个好现象，说明ai在干净的样本中表现良好  
2.负面情绪的数据不平衡问题依旧没有解决，通过混淆矩阵来看，除了happy，calm，surprised的表现都很堪忧  
未来优化方向：  
通过过采样/针对负面情绪进行增强产生变体样本来改变不均衡的样本分布  
做相似类区分：在模型中增加针对 “scared vs surprised”“sad vs calm” 的特征对比层（如加入注意力机制，让模型关注两类的差异特征）  
使用对在标注中的投票结果进行了放宽处理的数据集（可以举几个例子来说明这个情绪人也很难分辨情绪，所以放宽是合理的）
