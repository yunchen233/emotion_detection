...
def residual_block(x, filters, kernel_size=(3, 3)):
    ...
    大概的原理解释如下————
    残差块的数学表达：output = F(x) + x，x: 原始输入（跳跃连接），F(x): 网络学习的新特征，保留了原始信息，不会像顺序网络模糊很多细节
    残差块解决深层网络梯度消失问题，增强特征传递
    """
    # 主路径：两次卷积+批归一化
    residual = x# 保存原始输入
    x = Conv2D(filters, kernel_size, padding='same')(x)#第一个卷积层
    x = BatchNormalization()(x)#批归一化
    x = Activation('relu')(x)#激活函数，突出重要变化

    x = Conv2D(filters, kernel_size, padding='same')(x)#第二个卷积层
    x = BatchNormalization()(x)
    #第二个卷积层精炼特征，通过正负权重判断特征属性，如果再加上激活函数，负权重会被截断为0，网络只能增强特征无法抑制特征
...
train_generator = datagen.flow(X_train, y_train, batch_size=64)# 仅对训练集使用增强
print("--- 数据增强配置完成 ---")
...
# 新增了学习率调整：验证集损失停滞时降低学习率，精细优化。
lr_scheduler = ReduceLROnPlateau(
    monitor='val_loss',
    factor=0.5,  # 学习率减半
    patience=4,  # 4个epoch无改进则调整，比早停的epoch小，更精细
    min_lr=1e-6  # 最小学习率
)
